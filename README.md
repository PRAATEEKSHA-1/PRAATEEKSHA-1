# Hi, I'm Prateeksha 👋

### Data Analyst | M.Sc. Economics & Management, IIIT Lucknow | Turning raw data into business decisions

I'm a Data Analyst (M.Sc. Economics & Management, IIIT Lucknow) with hands-on experience in retail/inventory analytics, customer behavior analysis, and predictive modeling — currently working in a project-driven environment and exploring analytics roles across data-focused industries.

---

### 🔧 Technical Skills

### 💻**Programming & Analysis:** 
                              Python (Pandas, NumPy), SQL(Join, CTE, Window function, Subquery, Advanced Excel (Power Query, VLOOKUP/XLOOKUP, VBA)
### 📊**Visualization & BI:** 
                              Power BI (DAX), Excel Dashboards, PowerPoint,Power Query
### 📈**Statistics & Modeling:** 
                              Linear & Logistic Regression, Hypothesis Testing, ANOVA, Correlation Analysis, Time Series Analysis, VAR Modeling, Granger Causality, Stationarity Testing (ADF/KPSS)
### 🤝**Professional skills:** 
                               MIS Reporting, Data Validation & Cleaning, EDA, Feature Engineering, Stakeholder Reporting

---

### 📌 Featured Projects

**[Solving Inventory Inefficiencies Using Advanced SQL](https://github.com/PRAATEEKSHA-1/Solving_Inventory_Inefficiencies_Using_Advanced_SQL-)**
Built a normalized (3NF) inventory database for a retail chain and used advanced SQL (joins, window functions) + Power BI to identify stockout/overstock patterns, evaluate supplier performance, and forecast seasonal demand.

**[Viewer Retention & Churn Analysis for OTT Streaming Platforms](https://github.com/PRAATEEKSHA-1/Viewer-Retention-Churn-Analysis-for-OTT-Streaming-Platforms)**
Analyzed 33,000+ episode-level records across 23 behavioral features; used K-Means clustering and correlation analysis to identify what drives viewer drop-off vs. retention.

**[Medical Insurance Cost Analysis](https://github.com/PRAATEEKSHA-1/Medical_Insurance_Cost_Analysis)**
Analyzed 1,338 records to identify cost drivers; built Linear Regression (R²=0.84) and Random Forest (49% RMSE reduction) models to predict medical expenses and support risk segmentation.

**[Macroeconomic Analysis of HDFC Bank Stock Returns](https://github.com/PRAATEEKSHA-1/Analysis-of-Macroeconomic-Indicators-on-Stock-Market-Returns)**
Applied econometric time-series methods (ADF/KPSS stationarity tests, VAR modeling, Granger causality) to 15+ years of monthly data testing whether inflation, gold, oil, and money supply predict HDFC Bank stock returns. Found only a weak gold-price effect and no significant predictive relationships — a result consistent with the Efficient Market Hypothesis, demonstrating rigorous hypothesis testing over confirmation-seeking.

---

### 🌐 Let's Connect

[LinkedIn](https://www.linkedin.com/in/prateeksha-singh-1738a7302) • [Email](mailto:prateekshasinghrajpoot@gmail.com)

-------------------------------------------------------------------------------------------------

